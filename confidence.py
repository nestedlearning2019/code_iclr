...
fine_conf = np.mean(np.max(ps,axis = 1))
middle_conf = np.mean(np.max(pms,axis = 1))
coarse_conf = np.mean(np.max(resc_scaled,axis = 1))

print(coarse_conf)
print(middle_conf)
print(fine_conf)

df.loc[df['perturbation'] == per_name,'coarse_conf'] = coarse_conf
df.loc[df['perturbation'] == per_name,'middle_conf'] = middle_conf
df.loc[df['perturbation'] == per_name,'fine_conf'] = fine_conf
